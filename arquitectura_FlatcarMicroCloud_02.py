...
with Diagram("FlatcarMicroCloud — Vista Lógica (K3s HA + HAProxy/Keepalived)", show=False, direction="TB", outformat="png"):
    ...
    host_fw = Internet("Host Hypervisor + Firewall (nftables)\nKVM/libvirt · 192.168.0.40")
    ...
    host_fw >> Edge(label="LAN 192.168.0.0/24") >> [vip_api, vip_web]
    ...
    router >> Edge(label="8080/2052") >> vip_web

    # VIPs anuncian hacia el par de LBs
    vip_api >> [lb1, lb2]
    vip_web >> [lb1, lb2]

    # Plano de control (masters K3s) – red 10.17.4.0/24
    with Cluster("Masters K3s (10.17.4.0/24)"):
        m1 = K3S("master1\n10.17.4.21")
        m2 = K3S("master2\n10.17.4.22")
        m3 = K3S("master3\n10.17.4.23")

    # Workers + Storage (incluye Longhorn/NFS) – red 10.17.4.0/24
    with Cluster("Workers + Storage (10.17.4.0/24)"):
        w1 = LinuxGeneral("worker1\n10.17.4.24")
        w2 = LinuxGeneral("worker2\n10.17.4.25")
        w3 = LinuxGeneral("worker3\n10.17.4.26")
        storage = LinuxGeneral("storage1 (Longhorn + NFS)\n10.17.4.27")

    # Ingress L7 Traefik dentro del clúster (expuesto por NodePort/HostPort)
    ...
    # Servicios base (Infra) – red 10.17.3.0/24
    with Cluster("Servicios Base (10.17.3.0/24)"):
        dns_ntp = Coredns("CoreDNS + Chrony\n10.17.3.11")
        pg = Postgresql("PostgreSQL\n10.17.3.14")

    # Dependencias lógicas: DNS/NTP para nodos y DB para workloads
    [m1, m2, m3, w1, w2, w3] >> Edge(label="DNS/NTP") >> dns_ntp
